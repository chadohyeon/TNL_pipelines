...
### 1. BWA-MEM + sortedBAM + MarkDuplicateSpark
cat > ${PBS_DIR}/${sample}_01-1.align_sort_mark.${DATE}.pbs << EOF
#PBS -N ${sample}_01-1.align_sort_mark.${DATE}
#PBS -j oe
#PBS -q ${Queue}
#PBS -o ${PBS_LOGS}/${sample}_01-1.align_sort_mark.${DATE}.stdout.txt
...
echo "QUERY:02.MarkDuplicatesSpark:Sample=${sample}"
${gatk} --java-options "-Xmx${MEM}g -Djava.io.tmpdir=${JAVA_SPACE}" MarkDuplicatesSpark\
 ...
 --conf "spark.local.dir=${SPARK_SPACE}"
sleep 10
exit 0

EOF
FIRST_JOB=`qsub ${PBS_DIR}/${sample}_01-1.align_sort_mark.${DATE}.pbs`



### 2. BQSR Scattered jobs
SECOND_MULTI_JOBS=""
allBQSR_Bams=""
...
cat > ${PBS_DIR}/${sample}_01-2.BQSR_chunk0000.${DATE}.pbs << EOF
#PBS -N ${sample}_01-2.BQSR_chunk0000.${DATE}
#PBS -j oe
#PBS -q ${Queue}
#PBS -o ${PBS_LOGS}/${sample}_01-2.BQSR_chunk0000.${DATE}.stdout.txt
#PBS -l nodes=1:ppn=1
...
SECOND_MULTI_JOBS+=":"`qsub -W depend=afterok:${FIRST_JOB} ${PBS_DIR}/${sample}_01-2.BQSR_chunk0000.${DATE}.pbs`

if [ "${Cores}" -gt 1 ]; then
for itr in `seq -f %04g 1 $(expr ${Cores} - 1)`; do

cat > ${PBS_DIR}/${sample}_01-2.BQSR_chunk${itr}.${DATE}.pbs << EOF
#PBS -N ${sample}_01-2.BQSR_chunk${itr}.${DATE}
#PBS -j oe
#PBS -q ${Queue}
#PBS -o ${PBS_LOGS}/${sample}_01-2.BQSR_chunk${itr}.${DATE}.stdout.txt
#PBS -l nodes=1:ppn=1
...
SECOND_MULTI_JOBS+=":"`qsub -W depend=afterok:${FIRST_JOB} ${PBS_DIR}/${sample}_01-2.BQSR_chunk${itr}.${DATE}.pbs`
done
fi

### 3. Gathering BQSR scatters
cat > ${PBS_DIR}/${sample}_01-3.Gather_BQSR.${DATE}.pbs << EOF
#PBS -N ${sample}_01-3.Gather_BQSR.${DATE}
#PBS -j oe
#PBS -q ${Queue}
#PBS -o ${PBS_LOGS}/${sample}_01-3.Gather_BQSR.${DATE}.stdout.txt
#PBS -l nodes=1:ppn=1
...
THIRD_JOB=`qsub -W depend=afterok${SECOND_MULTI_JOBS} ${PBS_DIR}/${sample}_01-3.Gather_BQSR.${DATE}.pbs`
